...
def binary_edge_metrics(y_true_log: np.ndarray, y_pred_log: np.ndarray, threshold: float, mask: np.ndarray | None = None, prefix: str = "") -> dict[str, float]:
    ...
    metrics[f"{metric_prefix}f1@tau"] = metrics[f"{metric_prefix}f1_at_tau"]
    return metrics
...
def grouped_regression_metrics(
    ...
    for name, mask in groups.items():
        if np.sum(mask) == 0:
            ...
            if name == "positive_off_diagonal":
                metrics["positive_off_diagonal_only_mae"] = float("nan")
                metrics["positive_off_diagonal_only_rmse"] = float("nan")
            continue
        ...
        if name == "positive_off_diagonal":
            metrics["positive_off_diagonal_only_mae"] = metrics[f"{name}_mae"]
            metrics["positive_off_diagonal_only_rmse"] = metrics[f"{name}_rmse"]
    return metrics
...
def compute_all_metrics(sample: dict[str, Any], pred_log: np.ndarray, threshold: float, top_k: int, distance_bins: list[float]) -> dict[str, Any]:
    ...
    for k in (1, 3, 5, 10):
        recall = top_k_recall(y_true, pred_log, top_k=k)
        metrics[f"top_k_recall_at_{k}"] = recall
        metrics[f"top_k_recall@{k}"] = recall
