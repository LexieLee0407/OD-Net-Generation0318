...
class PairConditionHead(nn.Module):
    def __init__(self, hidden_dim: int, pair_dim: int, geo_dim: int = 6, dropout: float = 0.1) -> None:
        super().__init__()
    ...
    def forward(self, node_repr: torch.Tensor, pair_condition_geo: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pair_features = torch.cat([left, right, left * right, left - right, pair_condition_geo], dim=-1)
...
class GraphGPSRegressor(nn.Module):
    def __init__(
        ...
        self.pair_head = PairConditionHead(hidden_dim=hidden_dim, pair_dim=pair_dim, geo_dim=6, dropout=dropout)

    def _pair_condition_geo(self, sample: dict[str, torch.Tensor]) -> torch.Tensor:
        """按固定顺序构造 `g_ij=[log(d), dx, dy, selfloop, log(pop_i), log(pop_j)]`。"""

        if "pair_geo" not in sample or "pair_baseline" not in sample:
            raise KeyError("GraphGPS regressor 需要 pair_geo 与 pair_baseline。")
        return torch.cat([sample["pair_geo"], sample["pair_baseline"][..., :2]], dim=-1)
    ...
    def forward(self, sample: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        cond_map, pred = self.pair_head(x, self._pair_condition_geo(sample))
